...
def setup_logging(output_dir):
    ...
    log_file = os.path.join(output_dir, 'network_fc_analysis.log')
    logging.basicConfig(
        level=logging.INFO,
...
parser = argparse.ArgumentParser(description='Network-level pairwise FC analysis')
...
def load_network_fc_data(subject_ids, session, input_dir):
    ...
    logging.info("Loading FC data for %d subjects, session %s", len(subject_ids), session)
    fc_data = []
    ...
    for sid in subject_ids:
        ...
        if not os.path.exists(fc_path):
            logging.warning("FC file not found: %s", fc_path)
            continue
        try:
            fc_df = pd.read_csv(fc_path)
            logging.debug("Loaded FC file %s with %d rows", fc_path, len(fc_df))
            # Create unique feature identifier (sorted to avoid duplicates like A_B vs B_A)
            fc_df['feature_id'] = fc_df.apply(
                lambda x: '_'.join(sorted([x['network1'], x['network2']])) + '_fc', axis=1
            )
            ...
            logging.error("Failed to process FC file %s: %s", fc_path, e)
            continue
    if not fc_data:
        logging.warning("No FC data loaded for session %s", session)
        return pd.DataFrame(), feature_columns
    fc_data_df = pd.concat(fc_data, ignore_index=True)
    logging.info("Loaded FC data for %d subjects, %d features", len(fc_data_df), len(feature_columns))
...
def main():
    ...
    if baseline_fc_data.empty:
        logging.warning("No baseline FC data loaded. Exiting.")
        return
    ...
    if valid_longitudinal:
        ...
        logging.info("Baseline FC for OCD: n=%d", len(baseline_fc_ocd))
        ...
        for sid in valid_longitudinal:
            ...
            if not (os.path.exists(base_path) and os.path.exists(follow_path)):
                logging.warning("Missing FC files for subject %s (baseline: %s, followup: %s)",
                               sid, os.path.exists(base_path), os.path.exists(follow_path))
                continue
            try:
                base_fc = pd.read_csv(base_path)
                follow_fc = pd.read_csv(follow_path)
                logging.debug("Loaded baseline FC (%d rows) and followup FC (%d rows) for %s",
                ...
                logging.error("Failed to process longitudinal FC for subject %s: %s", sid, e)
                continue
        ...
        if fc_change_data:
            ...
            logging.info("Loaded FC change data for %d subjects, %d features",
            ...
            logging.warning("No FC change data loaded for longitudinal analysis")

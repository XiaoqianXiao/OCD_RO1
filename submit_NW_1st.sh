...
# SLURM job submission script for running NW_1st.py (ROI-to-ROI and ROI-to-network FC analysis)
# Updated to work with the revised NW_1st.py script
...
# Directory configuration (matching NW_1st.py DEFAULT_CONFIG)
...
check_dependencies() {
    ...
    # Check if NW_1st.py exists in the expected location
    if [ ! -f "/scratch/xxqian/repo/OCD_RO1/NW_1st.py" ]; then
        error_exit "NW_1st.py script not found at /scratch/xxqian/repo/OCD_RO1/NW_1st.py"
    fi
...
create_job_script() {
    ...
    local job_name="NW_1st_${subject}"
    local job_script="$TEMP_JOB_DIR/${job_name}.slurm"
    
    log_message "Creating job script for $subject..."
    
    # Define bind paths for Apptainer
    local apptainer_bind="/project/6079231/dliang55/R01_AOCD:/project/6079231/dliang55/R01_AOCD,/scratch/xxqian:/scratch/xxqian,/scratch/xxqian/repo/OCD_RO1/NW_1st.py:/app/NW_1st.py"
...
if apptainer exec --bind "${apptainer_bind}" "${CONTAINER}" python3 /app/NW_1st.py --subject ${subject} --verbose; then
...
main() {
    log_message "Starting NW_1st.py job submission process..."
